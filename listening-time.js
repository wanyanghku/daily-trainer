const VERSION='25';
const MODE=document.body.dataset.dictation==='number'?'number':'time';
const CONFIGS={
  time:{
    storeKey:'ielts_listening_time_v1',dataFile:'data-listening-time.json',expected:50,
    kicker:'今天的听写任务',title:'时间系列',titleTail:'50题一次过',sessionLabel:'时间系列 50题',startLabel:'开始 50 题',
    intro:'听音后直接输入答案。月份、星期和序数词要求完整拼写；日期与时间接受常见 IELTS 写法。所有题目随机出现。',
    note:'默认 1× 英音 · 大小写不敏感 · 每题可重复播放',success:'本轮全对。今天的时间系列可以停止。',error:'时间系列载入失败',
    categories:{month:{label:'月份',total:12},weekday:{label:'星期',total:7},ordinal:{label:'序数词',total:21},date:{label:'日期',total:5},clock:{label:'时间',total:5}}
  },
  number:{
    storeKey:'ielts_listening_number_v1',dataFile:'data-listening-number.json',expected:20,
    kicker:'周末可选专项',title:'数字系列',titleTail:'20题混合测',sessionLabel:'数字专项 20题',startLabel:'开始 20 题',
    intro:'听到后直接输入数字。覆盖电话号码、价格、普通数量和年份；空格与千位逗号不敏感，价格接受带或不带货币符号。',
    note:'默认 1× 英音 · 电话号码保留开头 0 · 每题可重复播放',success:'本轮全对。今天的数字专项可以停止。',error:'数字专项载入失败',
    categories:{phone:{label:'电话号码',total:5},price:{label:'价格',total:5},quantity:{label:'普通数字',total:5},year:{label:'年份',total:5}}
  }
};
...
function normalize(value){
  let normalized=String(value||'').normalize('NFKC').toLowerCase().replace(/[’']/g,'');
  if(MODE==='time'){
    return normalized.replace(/a\.?m\.?/g,'am').replace(/p\.?m\.?/g,'pm')
      .replace(/(\d)\.(\d)/g,'$1:$2').replace(/[-–—]/g,' ').replace(/[,]/g,' ')
      .replace(/\s+/g,' ').trim();
  }
  return normalized.replace(/pounds?/g,'£').replace(/dollars?/g,'$').replace(/euros?/g,'€')
    .replace(/[,\s\-–—]/g,'').trim();
}
...
function renderIntro(){
  ...
  const coverage=Object.values(CATEGORIES).map(category=>`<div><strong>${category.total}</strong><span>${esc(category.label)}</span></div>`).join('');
  app.innerHTML=`<section class="intro">
    <p class="intro-kicker">${esc(CONFIG.kicker)}</p>
    <h1>${esc(CONFIG.title)}<br>${esc(CONFIG.titleTail)}</h1>
    <p class="intro-copy">${esc(CONFIG.intro)}</p>
    <div class="coverage cols-${Object.keys(CATEGORIES).length}" aria-label="题目构成">${coverage}</div>
    <div class="intro-actions">
      <button class="primary" type="button" onclick="${hasActive?'continueSession()':'startSession()'}">${hasActive?`继续本轮 · ${state.session.index+1}/${state.session.ids.length}`:esc(CONFIG.startLabel)}</button>
    ...
    <p class="intro-note">${esc(CONFIG.note)}</p>
...
function renderTest(){
  ...
  app.innerHTML=`<section class="test">
    <div class="test-head"><div><small>${s.mode==='wrong'?'错题重练':esc(CONFIG.sessionLabel)}</small><strong>${s.index+1} / ${s.ids.length}</strong></div><span class="test-kind">${esc(item.label)}</span></div>
...
function renderResult(result,results){
  ...
  app.innerHTML=`<section class="result">
    ...
    <p class="result-summary">${wrong.length?`已把 ${wrong.length} 题加入错题重练。目标是同一小类连续两轮达到 95%。`:esc(CONFIG.success)}</p>
    ${lines}
    ...
    <div class="result-actions"><button class="primary" type="button" onclick="startSession('wrong')" ${state.wrongIds.length?'':'disabled'}>重练错题 ${state.wrongIds.length}</button><button class="secondary" type="button" onclick="startSession()">${esc(CONFIG.startLabel)}</button></div>
  </section>`;
}
...
async function boot(){
  try{
    const response=await fetch(`${CONFIG.dataFile}?v=${VERSION}`,{cache:'no-store'});
    if(!response.ok)throw new Error(`HTTP ${response.status}`);
    items=await response.json();
    if(!Array.isArray(items)||items.length!==CONFIG.expected)throw new Error(`题库不是 ${CONFIG.expected} 题`);
    ...
    app.innerHTML=`<div class="error-state"><strong>${esc(CONFIG.error)}</strong><p>${esc(error.message)}</p><button class="secondary" onclick="location.reload()">重新加载</button></div>`;
  }
}
boot();
